...
TEST(DatafileTest, Datafile)
{
    ...
    ASSERT_TRUE(serialize_data.size() % DatafileConfig::kAlignSize == 0);

    DatafileView datafile_view(serialize_data.data(), serialize_data.size());
    string blob2((const char *)datafile_view.blob_data(), datafile_view.blob_size());
